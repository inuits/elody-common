...
if [ "$1" = 'arangod' ]; then
    ...
    echo "Running custom scripts ...Hang on..."
    $NUMACTL arangod --config /tmp/arangod.conf \
            --server.endpoint tcp://127.0.0.1:$ARANGO_INIT_PORT \
            --server.authentication false \
            --log.file /tmp/init-log \
    ...
    if ! kill -s TERM "$pid" || ! wait "$pid"; then
        echo >&2 'Running cutsom ArangoDB scripts failed.'
        exit 1
    fi

    set -- arangod "$@" --server.authentication="$AUTHENTICATION" --config /tmp/arangod.conf --query.require-with=true
else
    NUMACTL=""
fi
